...
gc = gspread.service_account(filename = 'telegram-383417-aeb956c7ced6.json')#токен для подключения библиотеки к созданному приложению 

sh = gc.open("ИТ-291 | Расписание") #открываем таблицу с расписанием

week = 1 #переключатель текущей недели (недописанно,хочу через запросы получать)
if week == 1: 
	worksheet = sh.worksheet("1 неделя") #если первая ,открываем лист с названием "1 неделя"
if week == 2:
	worksheet = sh.worksheet("2 неделя") #если вторая,открываем лист с названием "2 неделя"
if week == 3:
	worksheet = sh.worksheet("3 неделя") #если третья,открываем лист с названием "3 неделя"
if week == 4:
	worksheet = sh.worksheet("4 неделя") #если четвертая,открываем лист с названием "4 неделя"
